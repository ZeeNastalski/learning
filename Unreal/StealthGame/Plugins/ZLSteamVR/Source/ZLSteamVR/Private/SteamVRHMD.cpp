...
#include "ZLSteamVRFunctionLibrary.h"
...
//ZEROLATENCY
DEFINE_LOG_CATEGORY(LogZLSteamVR);
//~ZEROLATENCY
...
IMPLEMENT_MODULE( FSteamVRPlugin, ZLSteamVR )
...
// ZEROLATENCY
// same as GetCurrentPose but returns the pose based on uncalibrated position and orientation stored in tracking frame
bool FSteamVRHMD::GetCurrentUncalibratedPose(int32 DeviceId, FQuat& CurrentOrientation, FVector& CurrentPosition)
{
	uint32 SteamDeviceID = static_cast<uint32>(DeviceId);
	bool bHasValidPose = false;
	...
	if (SteamDeviceID < vr::k_unMaxTrackedDeviceCount)
	{
		CurrentOrientation = TrackingFrame.UncalibratedDeviceOrientation[SteamDeviceID];
		CurrentPosition = TrackingFrame.UncalibratedDevicePosition[SteamDeviceID];
...
{
	//ZEROLATENCY
	TArray<uint32> trackedControllerDeviceIndexes;
	uint32 hmdIndex = 0;
	bool bFoundHMD = false;
	//~ZEROLATENCTY
...
#endif
		//ZEROLATENCY
		if (GetTrackedDeviceType(i) == EXRTrackedDeviceType::Controller)
		{
			trackedControllerDeviceIndexes.Add(i);
		}
		else if (GetTrackedDeviceType(i) == EXRTrackedDeviceType::HeadMountedDisplay)
		{
			hmdIndex = i;
			bFoundHMD = true;
		}
	...
	//ZEROLATENCY
	// save device positions and orientations to their uncalibrated counterparts before modifying anything in the TrackingFrame
	for (auto i = 0; i < vr::k_unMaxTrackedDeviceCount; i++)
	{
		FVector& uncalibratedDevicePosition = TrackingFrame.UncalibratedDevicePosition[i];
		uncalibratedDevicePosition = TrackingFrame.DevicePosition[i];

		FQuat& uncalibratedDeviceOrientation = TrackingFrame.UncalibratedDeviceOrientation[i];
		uncalibratedDeviceOrientation = TrackingFrame.DeviceOrientation[i];
	}

	// apply calibration
	if (bFoundHMD)
	{
		const auto& calibration = UZLCoreBlueprintFunctionLibrary::GetGameSpaceCalibration();

		const FVector uncalibratedHMDDevicePosition = TrackingFrame.DevicePosition[hmdIndex];

		FVector& hmdDevicePos = TrackingFrame.DevicePosition[hmdIndex];
		hmdDevicePos = UZLCoreBlueprintFunctionLibrary::SpaceMatrixTransformPosition(calibration.TrackingToContentMatrix, hmdDevicePos);

		FQuat& hmdDeviceOrientation = TrackingFrame.DeviceOrientation[hmdIndex];
		hmdDeviceOrientation = calibration.TrackingToContentQuat * hmdDeviceOrientation;

		// Backpack status contains 'WMR to Gun' offsets
		FZLBackpackStatus bpStatus = UZLCoreBlueprintFunctionLibrary::GetBackpackStatus();

		for (auto i = 0; i < trackedControllerDeviceIndexes.Num(); i++)
		{
			const uint32 trackedControllerIndex = trackedControllerDeviceIndexes[i];

			FVector& controllerDevicePosition = TrackingFrame.DevicePosition[trackedControllerIndex];			
			FQuat& controllerDeviceOrientation = TrackingFrame.DeviceOrientation[trackedControllerIndex];

			if (bpStatus.IsControllerHybrid)
			{
				// apply 'WMR to Gun' offset to left and right controller before applyig the calibration			
				if (trackedControllerIndex == 1)
				{
					// left - rotate the offset vector first to be in controller's local reference system
					controllerDevicePosition += controllerDeviceOrientation.RotateVector(bpStatus.LeftControllerTranslationOffset);
					controllerDeviceOrientation *= bpStatus.LeftControllerRotationOffset;
				}

				if (trackedControllerIndex == 2)
				{
					// right - rotate the offset vector first to be in controller's local reference system					
					controllerDevicePosition += controllerDeviceOrientation.RotateVector(bpStatus.RightControllerTranslationOffset);
					controllerDeviceOrientation *= bpStatus.RightControllerRotationOffset;
				}
			}
			
			// Apply calibration to the controler
			controllerDevicePosition = hmdDevicePos + calibration.TrackingToContentQuat.RotateVector(controllerDevicePosition - uncalibratedHMDDevicePosition);
			controllerDeviceOrientation = calibration.TrackingToContentQuat * controllerDeviceOrientation;
		}
	}
	//~ZEROLATENCY
}
...
bool FSteamVRHMD::OnStartGameFrame(FWorldContext& WorldContext)
{
	...
	// ZEROLATENCY	
	// Collecting headset's position and rotation (both calibrated and uncalibrated) 
	// and pushing it to the Launcher.
	FQuat TrackingOrientation;
	FVector TrackingPosition;
	GetCurrentUncalibratedPose(IXRTrackingSystem::HMDDeviceId, TrackingOrientation, TrackingPosition);

	FZLPlayerTrackingData playerInfo;
	playerInfo.ArenaHeadOrientation = Orientation;
	playerInfo.ArenaHeadPosition = Position;
	playerInfo.TrackingHeadPosition = TrackingPosition;
	playerInfo.TrackingHeadOrientation = TrackingOrientation;

	EnumerateTrackedDevices(playerInfo.TrackedControllers, EXRTrackedDeviceType::Controller);	
	UZLCoreBlueprintFunctionLibrary::SetPlayerTrackingUpdate(playerInfo);
	// ~ZEROLATENCY
