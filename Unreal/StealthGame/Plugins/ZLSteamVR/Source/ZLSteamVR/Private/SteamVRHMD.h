...
public:
	...
	// ZEROLATENCY
	virtual bool GetCurrentUncalibratedPose(int32 DeviceId, FQuat& CurrentOrientation, FVector& CurrentPosition);
	// ~ZEROLATENCY
...
public:
	...
	ZLSTEAMVR_API ETrackingStatus GetControllerTrackingStatus(int32 DeviceId) const;
...
private:
	...
	float GameWorldToMetersScale;
 	...
 	struct FTrackingFrame
 	{
		...
		// ZEROLATENCY
		FVector UncalibratedDevicePosition[vr::k_unMaxTrackedDeviceCount];
		FQuat UncalibratedDeviceOrientation[vr::k_unMaxTrackedDeviceCount];
		// ~ZEROLATENCY
			...
			, WorldToMetersScale(100.0f)
		{
			...
			// ZEROLATENCY
			// Initializing uncalibrated position and orientation
			FMemory::Memzero(UncalibratedDevicePosition, MaxDevices * sizeof(FVector));

			for (uint32 i = 0; i < vr::k_unMaxTrackedDeviceCount; ++i)
			{
				UncalibratedDeviceOrientation[i] = FQuat::Identity;
			}
			// ~ZEROLATENCY
